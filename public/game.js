class GomokuGame {
    ...
    constructor() {
        ...
        this.traps = []; // [{row, col, playerId (who placed it)}]
        this.cardEffectInProgress = false;
        // pendingCardUse now may contain 'targets' array for multi-target cards
        this.pendingCardUse = null; // { card: obj, cardIndex: num, type: 'trap'/'obstacle'/..., targets?: [] }
    ...
    getCardDefinitions() {
        return {
            'convert': { name: '棋子轉化', description: '選擇對方一個棋子轉化為我方 (不能因此直接獲勝)。', icon: '🔄' },
            'obstacle': { name: '障礙設置', description: '在棋盤的兩個空位放置障礙物。', icon: '🚧' },
            'storm': { name: '棋盤風暴', description: '隨機移動場上3-5顆棋子到其他空位。', icon: '🌪️' },
            'undo': { name: '悔棋', description: '取消你或對方的上一步落子，你重新下棋。', icon: '↩️' },
            'trap': { name: '陷阱佈置', description: '秘密選擇一個空位，對方下在此處棋子移除且跳過回合。', icon: '🕳️' },
            'hint': { name: '神機妙算', description: '系統高亮一個建議的落子點。', icon: '💡' },
    ...
    initiateCardUse(card, cardIndex) {
        ...
        switch (card.type) {
            ...
            case 'trap':
                this.cardEffectInProgress = true;
                const typeMap = {'convert': '轉換目標棋子', 'trap': '佈置陷阱'};
                this.showEffectMessage(`選擇棋盤位置以${typeMap[card.type]} (按ESC取消)`);
                this.canvas.style.cursor = 'crosshair';
                break;
            case 'obstacle':
                this.pendingCardUse.targets = []; // Initialize array to hold the two targets
                this.cardEffectInProgress = true;
                this.showEffectMessage('選擇第 1 個障礙物位置 (共 2 個) (按ESC取消)');
                this.canvas.style.cursor = 'crosshair';
                break;
            case 'storm':
            case 'undo': // Undo needs server's last move, client doesn't pick target
    ...
    handleCardEffectVisualization(data) {
        ...
        switch (data.effect) { // 'effect' is card.type from server
            case 'convert':
                ...
                break;
            case 'obstacle':
                // Server now sends an array of targets in data.target
                const positions = data.target.map(t => `(${t.row}, ${t.col})`).join(' 和 ');
                message += ` 障礙物已放置於 ${positions}。`;
                data.target.forEach(t => {
                    this.highlightPosition(t.row, t.col, '#FF0000', 2000);
                });
                break;
            case 'storm':
                message += ` ${data.moved.length}顆棋子被風暴移動了！`;
    ...
    finishCardEffectSelection() {
        const wasInProgress = this.cardEffectInProgress;
        this.cardEffectInProgress = false;
        this.pendingCardUse = null;
        this.canvas.style.cursor = 'default';
        this.effectArea.style.display = 'none';
        if (wasInProgress) {
            this.drawBoard(); // Redraw to clear any temporary visuals (like the first obstacle marker)
        }
    }
    ...
    handleCanvasClick(e) {
        ...
        // Handle card effect selections
        if (this.cardEffectInProgress && this.pendingCardUse) {
            const cardType = this.pendingCardUse.type;

            // NEW: Logic for placing two obstacles
            if (cardType === 'obstacle') {
                const { targets, cardIndex } = this.pendingCardUse;
                let reason = '';
                if (this.board[row][col] !== 0) reason = "此處已有棋子！";
                else if (this.obstacles.some(obs => obs.row === row && obs.col === col)) reason = "此處已是障礙物！";
                else if (this.traps.some(t => t.row === row && t.col === col)) reason = "不能在陷阱上放置障礙物！";
                else if (targets.length === 1 && targets[0].row === row && targets[0].col === col) reason = "不能選擇相同的位置！";
                ...
                if (targets.length === 1) {
                    this.showEffectMessage('選擇第 2 個障礙物位置 (共 2 個) (按ESC取消)');
                    this.drawBoard(); // Redraw to show temporary marker for the first obstacle
                } else {
                    this.socket.emit('useCard', {
                        roomId: this.roomId,
                        cardIndex: cardIndex,
                        target: targets // Send array of 2 targets
                    });
                    this.finishCardEffectSelection();
                }
            ...
            // Original logic for single-target cards like 'convert' and 'trap'
            if (cardType === 'convert' || cardType === 'trap') {
                let canPlace = true;
                let reason = "";
                if (this.board[row][col] !== 0 && cardType !== 'convert') {
                     canPlace = false; reason = "此處已有棋子！";
    ...
    drawBoard() {
        ...
        // ADDED: Draw temporary obstacle marker if one has been selected
        if (this.cardEffectInProgress && this.pendingCardUse && this.pendingCardUse.type === 'obstacle' && this.pendingCardUse.targets.length === 1) {
            const tempObstacle = this.pendingCardUse.targets[0];
            const x = 20 + tempObstacle.col * 40;
            const y = 20 + tempObstacle.row * 40;
            this.ctx.globalAlpha = 0.6; // Use globalAlpha for transparency
